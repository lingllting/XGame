...
public class Crc32 
{
	...
	public static string CRC32Str(byte[] dat)
	{
		UInt32 value = Crc32.CRC32(dat, dat.Length);
		return Convert.ToString(value, 16).ToUpper();
	}
	public static UInt32 CRC32(byte[] dat, int count)
	{
		int num2 = 0;
		...
		{
			count--;
			crc = ((crc >> 8) & 0x00FFFFFF) ^ crcTable[(crc ^ dat[num2]) & 0xFF];
			num2++;
		}
		crc = crc ^ 0xFFFFFFFF;
		return crc;
